<template>
  <div class="min-h-screen bg-gray-50 py-8 px-4">
    <div class="max-w-5xl mx-auto bg-white rounded-lg shadow-md">
      <Toast />
      <div class="p-6">
        <h1 class="text-2xl font-semibold text-center text-gray-800 mb-8">Education</h1>

        <div class="space-y-8">
          <div v-for="(education, index) in educations" :key="index" class="border rounded-lg p-6 bg-gray-50">
            <Form
                :validation-schema="schema"
                :initial-values="education"
            ...
            >
              <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="flex flex-col gap-1">
                  ...
                  <Field name="college" v-slot="{ field, errorMessage }">
                    <InputText
                        v-bind="field"
                        type="text"
                  ...
                  <ErrorMessage name="college" v-slot="{ message }">
              ...
              <div class="flex flex-col gap-1">
                ...
                <Field name="qualification" v-slot="{ field, errorMessage }">
                  <InputText
                      v-bind="field"
                      type="text"
                ...
                <ErrorMessage name="qualification" v-slot="{ message }">
                  <Message severity="error" size="small" variant="simple">{{ message }}</Message>
                </ErrorMessage>
              </div>
...
const educations = ref(
    cvStore.education.length > 0
        ? cvStore.education.map(edu => ({
          college: edu.college || '',
...
const onFormSubmit = (index, values) => {
  ...
  if (index < cvStore.education.length) {
    cvStore.updateEducation(index, formattedEducation)
  } else {
    cvStore.addEducation(formattedEducation)
  }
...
onMounted(() => {
  if (cvStore.education.length > 0) {
    educations.value = cvStore.education.map(edu => ({
      ...edu,
      startDate: edu.startDate ? new Date(edu.startDate) : null,
      endDate: edu.endDate ? new Date(edu.endDate) : null
    }))
